...
namespace DuksGames.Argon.Gameplay
{
    ...
    /// </summary>
    public class RayCastingProximityClickManager : MonoBehaviour, 
        ISuspendable, 
        ...
        ICurrentBeaconProvider 
    {
        ...
        RaycastClickFinder _raycastClickFinder;

        void Awake()
        {
            this._raycastClickFinder = new RaycastClickFinder
            {
                Cam = this._camera,
            };
        }
        ...
        [SerializeField] bool DdontCast;
        [SerializeField] bool DdontChooseAmong;

        IEnumerator PollRaycasterOr()
        {
            while(this._shouldKeepPolling)
            {
                IChoosableClickBeacon choice = this.DdontCast ? null :
                         this._raycastClickFinder.UpdateCurrent()?.GetComponent<IChoosableClickBeacon>();
                if(choice == null)
                {
                    choice = this.DdontChooseAmong ? null : this.ChooseAmongCandidates();
                }
                this.UpdateWith(choice);
        ...
        void StartPolling()
        {
            if (!this._shouldKeepPolling)
            {
                ...
                StartCoroutine(this.PollRaycasterOr());
            }
        }

        void StopPolling() 
        { 
            this._shouldKeepPolling = false; 
        }
        ...
        // void UpdateCurrent()
        // {
        //     IChoosableClickBeacon choice = null;
        //     choice = this._raycastClickFinder.Current?.GetComponent<IChoosableClickBeacon>();
        //     if (choice == null)
        //     {
        //         choice = ChooseAmongCandidates();
        //         this.UpdateWith(choice);
        //     }
        // }

        IChoosableClickBeacon ChooseAmongCandidates()
        {
            ...
            return pick.CameraAlignmentRating < this._dotTolerance ? pick.Candidate : null;
        }
